# -*- coding: utf-8 -*-
"""
LSTM Very-Short-Term Load Forecast (5-min data → next 4 hours = 48 steps)

- Input CSV: load_5min_train_smoothed_normalized.csv with columns: timestamp,p_norm
- End-aligned windows: X[i-LB+1:i+1], y[i+1:i+1+H]
- Time-aware split (no horizon leakage)
- Metrics (MSE, MAE, R2), training curve (muestra 3 ventanas separadas por H)
- Guarda el mejor modelo (include_optimizer=False) y un JSON con la config/val metrics
...
HORIZON_MINUTES = 60 * 4      # 4 horas → 48 pasos a 5 min
LOOKBACK_HOURS  = 8.0         # 8 horas de historial → 96 pasos a 5 min
TRAIN_RATIO     = 0.75        # split temporal
...
def infer_step_minutes(index: pd.DatetimeIndex) -> int:
    ...
    if step_min <= 0:
        raise ValueError("No se pudo inferir un step size positivo.")
    return step_min
...
def build_end_aligned_windows(X, y, lookback_steps, horizon_steps, end_indices):
    n_feat = X.shape[1]
...
if "p_norm" not in df.columns:
    raise ValueError("El CSV debe contener la columna 'p_norm'.")

STEP_MIN = infer_step_minutes(df.index)  # debería ser 5
if HORIZON_MINUTES % STEP_MIN != 0:
    raise ValueError(f"HORIZON_MINUTES ({HORIZON_MINUTES}) debe ser múltiplo de {STEP_MIN}.")

H = HORIZON_MINUTES // STEP_MIN         # 240/5 = 48
LB = int(round(LOOKBACK_HOURS * 60.0 / STEP_MIN))  # 8h → 96
if LB < 1:
    raise ValueError("LOOKBACK_STEPS debe ser >= 1.")
...
# -------------------------
# Split temporal sin leakage de horizonte
# -------------------------
i_min = LB - 1
i_max = n_points - 1 - H
if i_max < i_min:
    raise ValueError("Datos insuficientes para el lookback/horizonte elegidos.")

boundary_idx = max(int(math.floor(n_points * TRAIN_RATIO)) - 1, 0)

# Train: i <= boundary_idx - H (que su objetivo no cruce la frontera)
train_i_min = i_min
...
if (len(train_end_indices) == 0) or (len(val_end_indices) == 0):
    raise ValueError(
        f"Split vacío. Sube TRAIN_RATIO o reduce HORIZON_MINUTES. "
...
print(f"Step: {STEP_MIN} min | LB: {LB} pasos | H: {H} pasos")
print(f"Puntos: {n_points} | Boundary idx: {boundary_idx} (t={df.index[boundary_idx]})")
...
model_name = f"lstm_vstf_load_{STEP_MIN}min_{HORIZON_MINUTES}min_{LB}"
...
# -------------------------
# Plot: 3 muestras separadas por H
# -------------------------
i0 = 2 * H  # elige un offset razonable
if i0 + 2*H < len(y_val):
    y_val_concat = np.concatenate([y_val[i0], y_val[i0 + H], y_val[i0 + 2*H]])
    y_pred_concat = np.concatenate([y_val_pred[i0], y_val_pred[i0 + H], y_val_pred[i0 + 2*H]])
    plt.figure(figsize=(10, 5))
    plt.plot(y_val_concat, label="True", marker='o')
    plt.plot(y_pred_concat, label="Pred", marker='x')
    plt.title(f"Validation samples @ i0={i0}, i0+H, i0+2H (H={H} steps)")
    plt.xlabel("Step within each horizon")
    plt.ylabel("Normalized Load")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(FIGURES_DIR, model_name + "_val_3xH.png"), dpi=150)
    plt.close()

# -------------------------
# Guardar config y modelo
# -------------------------
run_cfg = RunCfg(
    csv_path=CSV_PATH,
