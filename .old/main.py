...
# NUMBER_ITERS = 288  # 24h @ 5-min
NUMBER_ITERS = 288 * 10  # example: 10 days @ 5-min
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------
    # Track previous-step net BESS power to enforce first-step ramp
    # Convention: P_bess = P_dis - P_ch (positive = discharging)
    # Set 0.0 for the very first optimization call of the horizon.
    # ------------------------------------------------------------
    pbess_prev_kw = 0.0  # NEW
    ...
    while not env.done():
        ...
        if env.mode == "offgrid":
            ...
            # OffGridMPC (unchanged): no Pbess_hat here unless você também adaptou OffGridMPC
            ...
            # NEW: pass previous-step BESS net power to anchor the first-step ramp
            active_mpc.build(
                start_dt=current_time,
                forecasts=forecasts,
                E_hat_kwh=env.E_meas,
                P_bess_hat_kw=pbess_prev_kw  # <-- NEW (key change)
            )
            active_mpc.solve("gurobi", tee=False)
        ...
        # ------------------------------------------------------------
        # Update pbess_prev_kw with the actually applied/measured power
        # Prefer the environment's feedback if available; fallback to commanded P_bess.
        # ------------------------------------------------------------
